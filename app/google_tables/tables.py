...
def get_spreadsheet_id_from_database(url): #TODO: variables for method
    link = Tables.url
...
def get_link_id_from_database(url): #TODO: variables for method
    link = Tables.url
    splitted = link.split("https://docs.google.com/spreadsheets/d/")
    link = splitted[1]
    splitted = link.split("/edit#gid=")
...
spreadsheet_id = get_spreadsheet_id_from_database(url)
list_id = get_link_id_from_database(url)
...
def students_on_lesson_rating(ranges, sheet_counter):
...
def students_on_lesson(ranges, sheet_counter):
    ranges_name = [list_name + "!" + "A1:A" + str(sheet_counter)]
    results = service.spreadsheets().values().batchGet(spreadsheetId = spreadsheet_id, 
                                                   ranges = ranges_name, 
                                                   valueRenderOption = 'FORMATTED_VALUE',  
                                                   dateTimeRenderOption = 'FORMATTED_STRING').execute()
    rating = students_on_lesson_rating(ranges, sheet_counter)
    student_on_lesson = []
    for i in rating:
        if int(rating[i]) != 0:
            student_on_lesson.append(results[i])
